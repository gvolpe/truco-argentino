...
for(int i = TRUCO; i <= AL_MAZO; i++)
	...
	if(botones_flag[i] == ON) //Mostrar Activados
		{		
		f = (SDL_Rect) {0, i * ((b_enabled->h)/9), b_enabled->w, (b_enabled->h)/9};
		d = (SDL_Rect) {555+x, 100 + (y*40), 0, 0};
		SDL_BlitSurface(b_enabled, &f, pantalla, &d);		
		}
	if(botones_flag[i] == OFF) //Mostrar Desactivados
		{
		f = (SDL_Rect) {0, i * ((b_disabled->h)/9), b_disabled->w, (b_disabled->h)/9};
		d = (SDL_Rect) {555+x, 100 + (y*40), 0, 0};
...
if(!player)  
	...
	for(i=0;i<3;i++)
		{
		if(p_naipes[i]<40)	{palo[i] = ORO;}
		if(p_naipes[i]<30)	{palo[i] = ESPADA;}
		if(p_naipes[i]<20)	{palo[i] = COPA;}
		if(p_naipes[i]<10)	{palo[i] = BASTO;}
		}
...
if(player)
	...
	for(i=0;i<3;i++)
		{
		if(c_naipes[i]<40)	{palo[i] = 3;}
		if(c_naipes[i]<30)	{palo[i] = 2;}
		if(c_naipes[i]<20)	{palo[i] = 1;}
		if(c_naipes[i]<10)	{palo[i] = 0;}
		}
	/// Calculamos los tantos ///
	if(palo[0]==palo[1] && palo[0]==palo[2])  //Las tres naipes del mismo palo
		{
...
{
SDL_Rect f, d;
f = (SDL_Rect) {439, 50, 120, 30};
d = (SDL_Rect) {439, 50, 0, 0};
